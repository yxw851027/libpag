...
可以从 [release](https://github.com/Tencent/libpag/releases) 页面下载预编译的二进制库，也可以通过 Swift Package Manager 或 CocoaPods 接入。

使用 Swift Package Manager 时，在 Xcode 中选择 **File > Add Package Dependencies**，输入本仓库地址，
选择 `4.5.75-spm.1` 版本，然后将 `libpag` 产品添加到 App Target。Swift 代码中可直接导入：

```swift
import libpag
```

使用 CocoaPods 时，在 Podfile 中添加 libpag 依赖：
